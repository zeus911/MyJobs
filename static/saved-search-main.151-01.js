...
function save_digest_form(form) {
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data == '') {
                form_status('Saved!', form);
            } else {
                form_status('Something went wrong', form);
            }
            add_errors(data);
        }
    });
...
function form_status(status, form) {
    var delay = 5000;
    var timer;
    clearTimeout(timer);
    form.find('#saved').addClass('label label-info');
    $('#saved').text(status);
