...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_index('seo_country', ['name'])

        # Adding index on 'Country', fields ['abbrev_short']
        db.create_index('seo_country', ['abbrev_short'])

        # Adding index on 'State', fields ['name']
        db.create_index('seo_state', ['name'])

        # Adding unique constraint on 'State', fields ['name', 'nation']
        db.create_unique('seo_state', ['name', 'nation_id'])

        # Adding index on 'City', fields ['name']
        db.create_index('seo_city', ['name'])

        # Adding unique constraint on 'City', fields ['state', 'name']
        db.create_unique('seo_city', ['state_id', 'name'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'City', fields ['state', 'name']
        db.delete_unique('seo_city', ['state_id', 'name'])

        # Removing index on 'City', fields ['name']
        db.delete_index('seo_city', ['name'])

        # Removing unique constraint on 'State', fields ['name', 'nation']
        db.delete_unique('seo_state', ['name', 'nation_id'])

        # Removing index on 'State', fields ['name']
        db.delete_index('seo_state', ['name'])

        # Removing index on 'Country', fields ['abbrev_short']
        db.delete_index('seo_country', ['abbrev_short'])

        # Removing index on 'Country', fields ['name']
        db.delete_index('seo_country', ['name'])
    ...
    models = {
        ...
        'seo.city': {
            ...
            'state': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['seo.State']"})
        },
        'seo.configuration': {
        ...
        'seo.country': {
            ...
            'abbrev_short': ('django.db.models.fields.CharField', [], {'db_index': 'True', 'max_length': '2', 'null': 'True', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'name': ('django.db.models.fields.CharField', [], {'max_length': '255', 'db_index': 'True'})
        },
        'seo.facet': {
