...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        default_domains = ['www.my.jobs', 'disability.jobs', 'veterans.jobs']
        default_sites = orm.SeoSite.objects.filter(domain__in=default_domains)

        ThroughModel = orm.Company.prm_saved_search_sites.through
        companies = orm.Company.objects.filter(partner__isnull=False).distinct()

        relationships_to_create = []

        for company in companies:
            for site in default_sites:
                relationships_to_create.append((company.pk, site.pk))

            job_source_ids = company.job_source_ids.all()
            buids = job_source_ids.values_list('id', flat=True)
            sites = orm.SeoSite.objects.filter(business_units__id__in=buids)
            if sites.count() <= 5:
                for site in sites:
                    relationships_to_create.append((company.pk, site.pk))

        relationships_to_create = set(relationships_to_create)
        created_relationships = [ThroughModel(company_id=x[0], seosite_id=x[1])
                                 for x in relationships_to_create]
        ThroughModel.objects.bulk_create(created_relationships)
